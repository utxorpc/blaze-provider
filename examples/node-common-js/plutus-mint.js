// Example: Mint native assets using a Plutus script (CommonJS)
const {
  Bip32PrivateKey,
  mnemonicToEntropy,
  NetworkId,
...
async function main() {
  ...
  const redeemer = PlutusData.fromCore({
    constructor: 0n,
    fields: { items: [Buffer.from(paymentKeyHash, 'hex')] }
  });
  ...
  try {
    ...
    console.log("2. The redeemer format doesn't match the Aiken script expectations");
