...
import { Bip32PrivateKey, mnemonicToEntropy, NetworkId, wordlist, AssetName, PlutusData, PlutusV2Script, HexBlob, } from "@blaze-cardano/core";
...
async function main() {
    ...
    const plutusScript = new PlutusV2Script(plutusScriptCbor);
    const script = Core.Script.newPlutusV2Script(plutusScript);
    ...
    // Create simple redeemer - just your key hash as bytes
    // Your script will validate this matches the transaction signer
    const redeemer = PlutusData.newBytes(Buffer.from(paymentKeyHash, 'hex'));
    // Step #8: Build the transaction
    try {
        const tx = await blaze
            .newTransaction()
            // Mint tokens using Plutus script
            .addMint(policyId, new Map([[assetName, mintAmount]]), redeemer // Required for Plutus scripts
        )
            // Send some ADA to recipient
            .payLovelace(recipientAddress, 2000000n)
            // Send some minted tokens to recipient
            .payAssets(recipientAddress, makeValue(0n, [Core.AssetId.fromParts(policyId, assetName).toString(), 500n]))
